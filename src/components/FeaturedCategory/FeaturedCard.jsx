...
function FeaturedCard({item}) {
    ...
    return (
        ...
        <div className="card-body h-72 md:h-52">
                ...
                <Link onClick={scrolltop} className='hover:text-primary transition-all' to={`/productDetails/${item._id}/${encodeURIComponent(item.name).replace(/%20/g, "-")}`}>{item.name} {item?.bestseller ? <div className="badge bg-green-600 border-0 text-white top-40 right-3">{item?.sells}+</div> : ''}</Link>
            ...
            <div className='absolute top-5 right-4 flex flex-col items-center gap-3 w-10'>
                    ...
                    <div className="bg-white rounded-full  border-none text-3xl text-yellow-400 "><MdStars/></div>
            </div>
            <div className="absolute bottom-4 ">
                <div className=" ">
                    ...
                    <button className="btn rounded-full btn-primary text-white w-full md:w-56">Add to cart</button>                                    
                    </div>                               
                </div>
