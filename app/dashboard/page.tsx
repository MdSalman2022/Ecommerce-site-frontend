...
export default function DashboardPage() {
    const { data: dashboard, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/analytics/dashboard`, {
                headers: {
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {stats.map((stat) => (
                    <div key={stat.title} className="bg-card rounded-xl p-5 shadow-sm border border-border">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="text-right">
                            <div className="text-2xl font-bold text-foreground">
                                ৳{revenueChart?.reduce((sum: number, d: any) => sum + d.revenue, 0).toLocaleString()}
                            </div>
                            <div className="text-xs text-muted-foreground">Total Weekly Revenue</div>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={revenueChart}>
                                ...
                                <Tooltip 
                                    ...
                                    labelFormatter={(label) => new Date(label).toLocaleDateString('en-GB', { dateStyle: 'long' })}
                                />
                                <Area 
                                    type="monotone" 
                                    dataKey="revenue" 
                ...
                <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
                    ...
                    <div className="space-y-6">
                        {topCategories?.length > 0 ? topCategories.map((item: any, index: number) => (
                            <div key={item.name} className="flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-[10px] text-muted-foreground/60 mt-1">{item.value} units sold</p>
                                </div>
                            </div>
                        )) : (
                        ...
                        <tbody className="divide-y divide-border">
                            {recentOrders?.length > 0 ? recentOrders.map((order: any) => (
                                <tr key={order.id} className="hover:bg-accent/10 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-sm font-bold text-foreground">৳{order.amount?.toLocaleString()}</td>
