  ...
  useEffect(() => {
    if (orderIdParam && orders.length > 0) {
      const order = orders.find(
        (o) => o.orderId === orderIdParam || o._id === orderIdParam
      );
      if (order) {
  ...
  const handleSearch = async (
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/orders/track/${searchPhone}`
      );
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 mt-2">
            Enter your phone number to view your orders and their status
          </p>
        </div>
            ...
            <div className="p-6">
              ...
              {(() => {
                ...
                const getStatusTimestamp = (status: string) => {
                  const historyItem = selectedOrder.statusHistory?.find(
                    (h) => h.status === status
                  );
