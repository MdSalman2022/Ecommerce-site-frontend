...
export default function Footer() {
  ...
  const socialLinks = [
    { Icon: Facebook, href: social?.facebook, color: 'hover:bg-blue-600', show: !!social?.facebook },
    { Icon: Instagram, href: social?.instagram, color: 'hover:bg-pink-600', show: !!social?.instagram },
    { Icon: Youtube, href: social?.youtube, color: 'hover:bg-red-600', show: !!social?.youtube },
    { Icon: FaTiktok, href: social?.tiktok, color: 'hover:bg-black', show: !!social?.tiktok },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-gray-500 text-sm mb-4 max-w-sm">
              Your trusted destination for quality electronics and gaming peripherals at the best prices.
            </p>
          ...
          <div className="hidden md:block">
            ...
            <ul className="space-y-2">
              {categories.slice(0, 5).map((category) => (
                <li key={category._id}>
                  <Link href={`/category/${category.slug}`} className="text-sm text-gray-500 hover:text-primary transition-colors">
                    {category.name}
                  </Link>
                </li>
              ))}
              ...
              <div key={key} className="border-b border-gray-200">
                ...
                {expandedSection === key && (
                  <ul className="pb-3 space-y-2">
                    {links.map((link) => (
                      <li key={link.name}>
                        <Link href={link.href} className="text-sm text-gray-500 hover:text-primary">
                          {link.name}
                        </Link>
                      </li>
                    ))}
