...
function DashboardOrders() {
  ...
  useEffect(() => {
    if (activeTab === 'courier' && !syncLockRef.current) {
      ...
      handleSyncCourierStatus().finally(() => {
        // Allow re-syncing if they switch away and back, 
        // or we could keep it locked for the session. 
        // Let's reset after a delay to prevent spam but allow updates.
        setTimeout(() => { syncLockRef.current = false; }, 30000); 
      });
    }
  }, [activeTab]);
  ...
  React.useEffect(() => {
    if (!searchQuery && !statusFilter && statusFilter === 'all' && !dateFrom && !dateTo) {
  ...
  React.useEffect(() => {
    ...
    const hasFilters = searchQuery.trim() || statusFilter !== 'all' || dateFrom || dateTo;
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/api/orders/search?${params.toString()}`,
          {
            headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setLocalOrders(prev => prev.filter(order => !selectedIds.includes(order._id)));
      setSelectedIds([]);
    } catch (err) {
      console.error(err);
      setDialogMessage({
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'processing': return 'bg-purple-100 text-purple-700 hover:bg-purple-200';
      case 'returned': return 'bg-gray-100 text-gray-700 hover:bg-gray-200';
      default: return 'bg-orange-100 text-orange-700 hover:bg-orange-200';
    }
  };
  ...
  const getCourierStatusColor = (status: string) => {
    switch(status?.toLowerCase()) {
      ...
      case 'partial_delivered': return 'bg-emerald-100 text-emerald-700';
  ...
  const handleSendToCourier = async (orderId: string) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/admin/order/courier`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.success) {
        ...
        setDialogMessage({
          title: 'Error',
          description: data.message || 'Failed to send to courier. Please try again.'
        });
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSyncCourierStatus = async () => {
    const courierOrders = localOrders.filter(o => o.courierInfo?.consignmentId);
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/admin/order/sync-courier`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (data.success) {
        if (data.updatedOrders && data.updatedOrders.length > 0) {
          const updatedMap = new Map(data.updatedOrders.map((o: any) => [o._id, o]));
          setLocalOrders(prev => 
            prev.map(order => updatedMap.has(order._id) ? { ...order, ...updatedMap.get(order._id) } : order)
          );
        }
    ...
    <div className="py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-8">
        ...
        <div className="flex items-center gap-4 bg-card border border-border p-5 rounded-xl hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{localOrders.filter((o: any) => o.orderStatus === 'delivered').length}</p>
          </div>
        </div>
        <div className="flex items-center gap-4 bg-card border border-border p-5 rounded-xl hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{localOrders.filter((o: any) => o.courierInfo?.consignmentId).length}</p>
          </div>
        </div>
        <div className="flex items-center gap-4 bg-card border border-border p-5 rounded-xl hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{localOrders.filter((o: any) => o.orderStatus === 'cancelled' || o.orderStatus === 'returned').length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <Tabs value={activeTab} onValueChange={(value) => { setActiveTab(value); setCurrentPage(1);  setSelectedIds([]); }} className="w-auto">
            <TabsList className="mb-0">
              <TabsTrigger value="all">All ({localOrders.length})</TabsTrigger>
              <TabsTrigger value="abandoned">
                Abandoned ({localOrders.filter((o: any) => o.orderStatus === 'cancelled' || o.orderStatus === 'returned').length})
              </TabsTrigger>
              <TabsTrigger value="courier">
                Courier ({localOrders.filter((o: any) => o.courierInfo?.consignmentId).length})
              </TabsTrigger>
            </TabsList>
          </Tabs>
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={handleDelete}>Delete Selected</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handleUpdateStatus('pending')}>Mark Pending</DropdownMenuItem>
              ...
              <DropdownMenuItem onClick={() => handleUpdateStatus('cancelled')} className="text-red-600">Cancel Selected</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => setSortType('latest')}>Latest First</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortType('old')}>Oldest First</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {displayItems.map((order: any, index: number) => (
                  <TableRow key={order._id} className="hover:bg-muted/30 h-[64px]">
                    <TableCell>
                      <Checkbox
                    ...
                    <TableCell className="text-muted-foreground">{indexOfFirstPost + index + 1}</TableCell>
                    <TableCell 
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className={`h-7 text-xs px-3 rounded-full capitalize font-medium ${
                                getStatusColor(pendingStatusChanges[order._id] || order.orderStatus || 'pending')
                              }`}
                            >
                              {pendingStatusChanges[order._id] || order.orderStatus || 'pending'} 
                    ...
                    <TableCell>
                      {order.courierInfo?.consignmentId ? (
                        <div className="flex flex-col justify-center gap-1.5 h-full py-1">
                            <div className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full w-fit uppercase ${getCourierStatusColor(order.courierInfo.status)}`}>
                           ...
                           <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-mono leading-tight">
                              <span title={`Consignment ID: ${order.courierInfo.consignmentId}`}>#{order.courierInfo.consignmentId}</span>
                              <span className="text-gray-300">|</span>
                              <div 
                                className="cursor-pointer hover:text-primary transition-colors flex items-center gap-1"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigator.clipboard.writeText(order.courierInfo.trackingCode);
                                }}
                        ...
                        <div className="flex items-center gap-1.5 opacity-60">
                          ...
                          <span className="text-xs font-medium text-gray-500 italic">Delivered</span>
                        </div>
                      ) : (
                        <Button
                          size="sm"
