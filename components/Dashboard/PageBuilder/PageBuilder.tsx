...
export default function PageBuilderDashboard() {
    ...
    const { data: pageData, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/page-builder/${selectedPage}`, {
                headers: {
    ...
    const publishMutation = useMutation({
        mutationFn: async () => {
            const token = localStorage.getItem('accessToken');
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/page-builder/${selectedPage}/publish`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const resetMutation = useMutation({
        mutationFn: async () => {
            const token = localStorage.getItem('accessToken');
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/page-builder/${selectedPage}/reset`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    if (isLoading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-muted-foreground animate-pulse">Loading builder...</p>
            </div>
        );
    }

    // Don't generate IDs here - they should come from backend
    // Generating IDs on every render causes the edit bug!
    ...
    const handleMove = (index: number, direction: 'up' | 'down') => {
        ...
        // Swap orders
        [newSections[index].order, newSections[newIndex].order] = [newSections[newIndex].order, newSections[index].order];
    ...
    const handleSaveSectionEdit = () => {
        ...
        if (isNewSection) {
            ...
            updated = sections.map(s => 
                s.id === editingSection.id ? JSON.parse(JSON.stringify(editingSection)) : s
            );
        }
    ...
    const handleRemoveSection = (sectionId: string) => {
        ...
        if (confirm('Are you sure you want to remove this section?')) {
            ...
            if (updated.length === 0 && sections.length > 1) {
                toast.error('Error: Cannot delete all sections. Please refresh and try again.');
                return;
            }
    ...
    const renderEditor = () => {
        ...
        switch (editingSection.type) {
            case 'hero':
                return <HeroEditor config={editingSection.config} onChange={handleConfigChange} />;
            case 'brandMarquee':
                return <BrandMarqueeEditor config={editingSection.config} onChange={handleConfigChange} />;
            case 'categories':
                return <CategoryEditor config={editingSection.config} onChange={handleConfigChange} />;
            case 'products':
                return <ProductEditor config={editingSection.config} onChange={handleConfigChange} />;
            default:
                return <div className="p-4 text-center text-muted-foreground italic">No specialized editor available for this section type.</div>;
        }
    };

    const getSectionIcon = (type: string) => {
        switch (type) {
            case 'hero': return <Monitor className="w-4 h-4" />;
            case 'brandMarquee': return <RefreshCcw className="w-4 h-4" />;
            case 'categories': return <Layout className="w-4 h-4" />;
            case 'products': return <Plus className="w-4 h-4" />;
        ...
        <div className="space-y-6 max-w-5xl mx-auto pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-border">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        {publishMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
                        Publish Changes
                    </Button>
                </div>
            </div>
                ...
                <div className="flex items-center justify-between px-2">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuItem onClick={() => handleAddSection('brandMarquee')}>
                ...
                <div className="grid gap-3">
                    {sections.map((section, index) => (
                        <div 
                            ...
                            className={`group bg-white border rounded-xl p-4 shadow-sm hover:border-primary/50 transition-all ${!section.isVisible ? 'opacity-60 bg-gray-50' : ''}`}
                        >
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button 
                                        variant="ghost" 
                                        size="icon" 
                                        className={`h-8 w-8 ${section.isVisible ? 'text-blue-500' : 'text-gray-400'}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
            ...
            <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl flex items-start gap-3">
                ...
                <div>
                    <p className="text-sm font-medium text-blue-900">Current Status: {pageData?.isPublished ? 'Live & Published' : 'Draft (Unsaved changes)'}</p>
                    <p className="text-xs text-blue-700/80 mt-0.5 uppercase font-semibold">
                        Version {pageData?.metadata?.version} • Last edited by {pageData?.metadata?.lastEditedBy || 'Admin'}
                    </p>
                </div>
            </div>
        </div>
