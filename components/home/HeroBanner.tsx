                ...
                <SwiperSlide key={slide.id}>
                  <div
                    className={`bg-gradient-to-br ${
                      slide.bgGradient || "from-gray-50 to-gray-100"
                    } relative h-[280px] md:h-[350px] lg:h-[400px] flex items-center`}
                  >
          ...
          <div className="hidden lg:flex flex-col gap-3">
            {promoCards.map((card: any) => (
              <Link
                key={card.id}
                href={card.link || "#"}
                className={`relative rounded-xl overflow-hidden h-[128px] group/card ${
              ...
              >
                {card.image && (
                  <div className="absolute inset-0 bg-black/40 group-hover/card:bg-black/50 transition-colors" />
                )}
                <div className="relative z-10 p-4 h-full flex flex-col justify-end">
                  <p className="text-white/80 text-[10px] uppercase font-bold tracking-wider">
                    {card.subtitle}
                  </p>
                  <h3 className="text-white font-bold text-lg group-hover/card:text-primary transition-colors">
                    {card.title}
                  </h3>
                </div>
              </Link>
            ...
            {promoCards.map((card: any) => (
              <Link
                ...
                className={`rounded-lg overflow-hidden h-20 ${
                  card.bgColor || "bg-gray-800"
                } flex items-center justify-center border border-border shadow-sm`}
                style={
                  card.image
                    ? {
                        backgroundImage: `url(${card.image})`,
              ...
              >
                <div
                  className={
                    card.image
                      ? "bg-black/50 w-full h-full flex items-center justify-center p-2"
                      : "p-2"
                  }
                >
