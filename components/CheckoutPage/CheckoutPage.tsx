...
function CheckoutPage() {
  ...
  // Load current user info defaults
  const info: any = user || {};
  ...
  // Validation Checks
  const isGuestCheckoutDisabled =
    !user && ecommerce?.enableGuestCheckout === false;
  const isMinOrderNotMet =
    ecommerce?.minOrderAmount > 0 && cartTotal < ecommerce.minOrderAmount;
  const isMaxOrderExceeded =
    ecommerce?.maxOrderAmount > 0 && cartTotal > ecommerce.maxOrderAmount;
        ...
        <div className="flex flex-col items-center max-w-xl mx-auto">
          ...
          {!isGuestCheckoutDisabled && isMinOrderNotMet && (
            <div className="w-full bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg mb-6">
              ...
              <p className="text-sm">
                Your cart total is ৳{cartTotal.toLocaleString()}. You need to
                spend at least ৳{ecommerce.minOrderAmount.toLocaleString()} to
                checkout.
              </p>
              <Link
                href="/"
          ...
          {!isGuestCheckoutDisabled && isMaxOrderExceeded && (
            <div className="w-full bg-red-50 border border-red-200 text-red-700 p-4 rounded-lg mb-6">
              ...
              <p className="text-sm">
                Your cart total is ৳{cartTotal.toLocaleString()}. The maximum
                order limit is ৳{ecommerce.maxOrderAmount.toLocaleString()}.
                Please reduce your cart items.
              </p>
            </div>
          )}
